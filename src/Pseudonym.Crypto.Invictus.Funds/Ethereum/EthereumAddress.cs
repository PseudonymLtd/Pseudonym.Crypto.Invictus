...
namespace Pseudonym.Crypto.Invictus.Funds.Ethereum
{
    public readonly struct EthereumAddress : IEquatable<EthereumAddress>
    {
        ...
        public static bool TryParse(string hex, out EthereumAddress ethereumAddress)
        {
            var success = AddressRegex.IsMatch($"0x{hex ?? string.Empty}");

            ethereumAddress = success
                ? new EthereumAddress(hex)
                : Empty;

            return success;
        }

        public static EthereumAddress Parse(string hex)
        {
            return new EthereumAddress(hex);
        }
